...
# Import the file
filename = 'CXS-1_Catalog.fits' #input('Enter catalog filename: ');
...
tmp = sampler_pl.run_mcmc(pos_pl, 600)
...
samples_pl = sampler_pl.chain[:, 100:, :].reshape((-1, 2))
#print(samples.shape)
...
def lnL_th(theta, x, y, yerr):
    S, f= theta
    model_th = ((1-f)*S*(x/restfreq)**(-a_NT_guess)+f*S*(x/restfreq)**(-0.1))#*u.uJy
    inv_sigma2 = 1.0/(np.power(yerr,2))
...
tmp = sampler_th.run_mcmc(pos_th, 650)
...
samples_th = sampler_th.chain[:, 150:, :].reshape((-1, 3))
...
tmp = sampler_SA.run_mcmc(pos_SA, 650)
...
samples_SA = sampler_SA.chain[:, 150:, :].reshape((-1, 3))
...
samples_SA_FF = sampler_SA_FF.chain[:, 230:, :].reshape((-1, 4))
...
#---------------------------------------------------------------------------------------------------------------------------------------
#Synchrotron aging + Free-free with an exponential exp
...
#Synchrotron aging +free free with exponential exp
...
plt.title('CXS-1 (z=1.44)', fontsize=22);
...
plt.savefig('CXS-1_Flux_Density.pdf',  dpi=500, clobber=True);
#plt.show();
...
plt.savefig('CXS-1_Luminosity.pdf',  dpi=500, clobber=True);
#plt.show();
...
#Some logistics of the figure
#plt.title('CXS-1 (z=1.44)', fontsize=15);
plt.xlabel(r'$\nu_{rest}$ (GHz)', fontsize = 22);
plt.ylabel(r'Luminosity (W Hz$^{-1}$)', fontsize=22);
plt.xscale('log');
plt.yscale('log');
...
plt.yticks(fontsize=20)
plt.xticks(axis1,axis1,fontsize=20)
plt.savefig('CXS-1_pl.pdf',  dpi=500, clobber=True);
#plt.show();
...
plt.yticks(fontsize=20)
plt.xticks(axis1,axis1,fontsize=20)
plt.savefig('CXS-1_FF+S.pdf',  dpi=500, clobber=True);
#plt.show();
...
plt.yticks(fontsize=20)
plt.xticks(axis1,axis1,fontsize=20)
plt.savefig('CXS-1_SA.pdf',  dpi=500, clobber=True);
#plt.show();
...
plt.yticks(fontsize=20)
plt.xticks(axis1,axis1,fontsize=20)
plt.savefig('CXS-1_SA+FF.pdf',  dpi=500, clobber=True);
#plt.show();
...
plt.yticks(fontsize=20)
plt.xticks(axis1,axis1,fontsize=20)
plt.savefig('CXS-1_SA+FF+exp.pdf',  dpi=500, clobber=True);
plt.show();
