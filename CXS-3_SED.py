...
# Import the file
filename = 'CXS-3_Catalog.fits' #input('Enter catalog filename: ');
...
if 'Peak_Flux_610MHz' in catalog[1].columns.names:
	...
	E_flux610MHz = np.sqrt(data.field('E_Peak_Flux_610MHz')**2 + (0.05*data.field('Peak_Flux_610MHz')**2))*u.Jy;
...
frequency = np.linspace(0.325, 22, num=30)
...
def red_chi_pl(S, a):
	chi_pl = 0
	model = (S*(frequency_obs/restfreq)**(-a))#*uJy
...
tmp = sampler_SA.run_mcmc(pos_SA, 750)
...
samples_SA = sampler_SA.chain[:, 250:, :].reshape((-1, 3))
...
def lnL_SA_FF(theta, x, y, yerr):
    S_nu, f, a, nu_b = theta
    model_SA_FF = (f*S_nu*(x/restfreq)**(-0.1) + (1-f)*S_nu*(((x/restfreq)**(-a))/(1+(x/(nu_b))**(0.5))))#*u.uJy
    inv_sigma2 = 1.0/(np.power(yerr,2))
...
ndim_SA_FF, nwalkers = 4, 500
theta_SA_FF_guess = np.array([S_nu_SA_FF_guess, f_th_SA_FF_guess, a_SA_FF_guess, nu_b_SA_FF_guess])
...
for i in range(len(frequency)):
	MCMC_SA_FF = np.append(MCMC_SA_FF, (samples_SA_FF[:,0]*samples_SA_FF[:,1]*((frequency[i]/restfreq)**(-0.1))+(1-samples_SA_FF[:,1])*samples_SA_FF[:,0]*((frequency[i]/restfreq)**(-samples_SA_FF[:,2]))/(1+(frequency[i]/(samples_SA_FF[:,3]))**(0.5))).reshape(len(samples_SA_FF[:,0]), 1), axis = 1)
...
def red_chi_SA_FF(S, f, a, nu_b):
	chi_SA_FF = 0
	model = (f*S*(frequency_obs/restfreq)**(-0.1) + (1-f)*S*(((frequency_obs/restfreq)**(-a))/(1+(frequency_obs/(nu_b))**(0.5))))#*u.uJy
	for i in range(len(frequency_obs)):
		chi_SA_FF += ((flux[i]-model[i])/E_flux[i])**2
	red_chi_SA_FF = chi_SA_FF/(len(frequency_obs)-4)
...
def lnL_SA_FF_CO(theta, x, y, yerr):
    S_nu, f, a, nu_b = theta
    model_SA_FF_CO = (f*S_nu*(x/restfreq)**(-0.1) + (1-f)*S_nu*(x/restfreq)**(-a)*np.exp(-x/(nu_b)))#*u.uJy
    inv_sigma2 = 1.0/(np.power(yerr,2))
...
ndim_SA_FF_CO, nwalkers = 4, 500
theta_SA_FF_CO_guess = np.array([S_nu_SA_FF_CO_guess, f_th_SA_FF_CO_guess, a_SA_FF_CO_guess, nu_b_SA_FF_CO_guess])
...
tmp = sampler_SA_FF_CO.run_mcmc(pos_SA_FF_CO, 500)
...
samples_SA_FF_CO = sampler_SA_FF_CO.chain[:, 100:, :].reshape((-1, 4))
...
for i in range(len(frequency)):
	MCMC_SA_FF_CO = np.append(MCMC_SA_FF_CO, (samples_SA_FF_CO[:,0]*samples_SA_FF_CO[:,1]*((frequency[i]/restfreq)**(-0.1))+(1-samples_SA_FF_CO[:,1])*samples_SA_FF_CO[:,0]*(frequency[i]/restfreq)**(-samples_SA_FF_CO[:,2])*np.exp(-frequency[i]/(samples_SA_FF_CO[:,3]))).reshape(len(samples_SA_FF_CO[:,0]), 1), axis = 1)
...
def red_chi_SA_FF_CO(S, f, a, nu_b):
	chi_SA_FF_CO = 0
	model = (f*S*(frequency_obs/restfreq)**(-0.1) + (1-f)*S*(frequency_obs/restfreq)**(-a)*np.exp(-frequency_obs/(nu_b)))#*u.uJy
	for i in range(len(frequency_obs)):
		chi_SA_FF_CO += ((flux[i]-model[i])/E_flux[i])**2
	red_chi_SA_FF_CO = chi_SA_FF_CO/(len(frequency_obs)-4)
...
def lnL_AGN(theta, x, y, yerr):
    S_nu, f, a_SA, nu_b = theta
    model_AGN = (S_nu*f*(((x/restfreq)**(-a_SA))/(1+(x/(nu_b))**(0.5))) + (1-f)*S_nu*(x/restfreq)**(-5/2)*(1-np.exp(-(x/(nu_1_AGN_guess))**(-a_SSA_AGN_guess-5/2))))#*u.uJy
    inv_sigma2 = 1.0/(np.power(yerr,2))
    
    return -0.5*(np.sum((y-model_AGN)**2*inv_sigma2))

def lnprior_AGN(theta):
	S_nu, f, a_SA, nu_b = theta
    	if 0 < S_nu < 700 and 0 < f < 1 and 0 < a_SA < 1 and 1 < nu_b < 22:
        	return 0.0
    	return -np.inf

def lnprob_AGN(theta, x, y, yerr):
    lp_AGN = lnprior_AGN(theta)
    if not np.isfinite(lp_AGN):
        return -np.inf
    return lp_AGN + lnL_AGN(theta, x, y, yerr)
    
ndim_AGN, nwalkers = 4, 500
theta_AGN_guess = np.array([S_AGN_guess, f_AGN_guess, a_SA_AGN_guess, nu_b_AGN_guess])
pos_AGN = [theta_AGN_guess + 1e-4*np.random.randn(ndim_AGN) for i in range(nwalkers)]
sampler_AGN = emcee.EnsembleSampler(nwalkers, ndim_AGN, lnprob_AGN, args=(x, y_obs, dy))
tmp = sampler_AGN.run_mcmc(pos_AGN, 500)

fig, axes = plt.subplots(ncols=1, nrows=4)
fig.set_size_inches(12,12)
axes[0].plot(sampler_AGN.chain[:, :, 0].transpose(), color='black', alpha=0.3)
axes[0].set_ylabel(r'$S_{\nu}$')
axes[0].axvline(100, ls='dashed', color='red')
axes[1].plot(sampler_AGN.chain[:, :, 1].transpose(), color='black', alpha=0.3)
axes[1].set_ylabel(r'$f$')
axes[1].axvline(100, ls='dashed', color='red')
axes[2].plot(sampler_AGN.chain[:, :, 2].transpose(), color='black', alpha=0.3)
axes[2].set_ylabel(r'$\alpha_{SA}$')
axes[2].axvline(100, ls='dashed', color='red')
#axes[3].plot(sampler_AGN.chain[:, :, 3].transpose(), color='black', alpha=0.3)
#axes[3].set_ylabel(r'$\alpha_{SSA}$')
#axes[3].axvline(100, ls='dashed', color='red')
axes[3].plot(sampler_AGN.chain[:, :, 3].transpose(), color='black', alpha=0.3)
axes[3].set_ylabel(r'$\nu_b$')
axes[3].axvline(100, ls='dashed', color='red')
#axes[5].plot(sampler_AGN.chain[:, :, 5].transpose(), color='black', alpha=0.3)
#axes[5].set_ylabel(r'$\nu_{1}$')
#axes[5].axvline(100, ls='dashed', color='red')
fig.savefig('chain_AGN.pdf', format='pdf')
#plt.close()

samples_AGN = sampler_AGN.chain[:, 100:, :].reshape((-1, 4))


fig = corner.corner(samples_AGN, labels=[r"$S_{\nu}$", r"$f$", r"$\alpha_{SA}$", r"$\nu_b$"],quantiles=[0.16, 0.50, 0.84], show_titles=True)#truths=[S_nu0_guess, f_th_guess, a_NT_guess])
fig.savefig('corner_AGN.pdf', format='pdf')
#plt.close()

median_S_AGN = np.percentile(samples_AGN[:, 0], 50.0)
median_f_AGN = np.percentile(samples_AGN[:, 1], 50.0)
median_a_SA_AGN= np.percentile(samples_AGN[:, 2], 50.0)
#median_a_SSA_AGN = np.percentile(samples_AGN[:, 3], 50.0)
median_nu_b_AGN = np.percentile(samples_AGN[:, 3], 50.0)
#median_nu_1_AGN = np.percentile(samples_AGN[:, 5], 50.0)

p16_S_AGN = np.percentile(samples_AGN[:, 0], 16)
p16_f_AGN = np.percentile(samples_AGN[:, 1], 16)
p16_a_SA_AGN= np.percentile(samples_AGN[:, 2], 16)
#p16_a_SSA_AGN = np.percentile(samples_AGN[:, 3], 16)
p16_nu_b_AGN = np.percentile(samples_AGN[:, 3], 16)
#p16_nu_1_AGN = np.percentile(samples_AGN[:, 5], 16)

p84_S_AGN = np.percentile(samples_AGN[:, 0], 84)
p84_f_AGN = np.percentile(samples_AGN[:, 1], 84)
p84_a_SA_AGN= np.percentile(samples_AGN[:, 2], 84)
#p84_a_SSA_AGN = np.percentile(samples_AGN[:, 3], 84)
p84_nu_b_AGN = np.percentile(samples_AGN[:, 3], 84)
#p84_nu_1_AGN = np.percentile(samples_AGN[:, 5], 84)


sigma_S_AGN = 0.5*(p84_S_AGN-p16_S_AGN)
sigma_f_AGN = 0.5*(p84_f_AGN-p16_f_AGN)
sigma_a_SA_AGN = 0.5*(p84_a_SA_AGN-p16_a_SA_AGN)
#sigma_a_SSA_AGN = 0.5*(p84_a_SSA_AGN-p16_a_SSA_AGN)
sigma_nu_b_AGN = 0.5*(p84_nu_b_AGN-p16_nu_b_AGN)
#sigma_nu_1_AGN = 0.5*(p84_nu_1_AGN-p16_nu_1_AGN)


MCMC_AGN=np.empty(shape=[len(samples_AGN[:,0]), 0])
for i in range(len(frequency)):
	MCMC_AGN = np.append(MCMC_AGN, (samples_AGN[:,0]*samples_AGN[:,1]*(((frequency[i]/restfreq)**(-samples_AGN[:,2]))/(1+(frequency[i]/(samples_AGN[:,3]))**(0.5))) + (1-samples_AGN[:,1])*samples_AGN[:,0]*(frequency[i]/restfreq)**(-5/2)*(1-np.exp(-(frequency[i]/(nu_1_AGN_guess))**(-a_SSA_AGN_guess-5/2)))).reshape(len(samples_AGN[:,0]), 1), axis = 1)

AGN_16 = []
AGN_84 = []
for i in range(len(frequency)):
	AGN_16.append(np.percentile(np.sort(MCMC_AGN[:,i]),16))
	AGN_84.append(np.percentile(np.sort(MCMC_AGN[:,i]),84))
AGN_16 = np.array(AGN_16)*u.uJy
AGN_84 = np.array(AGN_84)*u.uJy

def red_chi_AGN(S_nu, f, a_SA, a_SSA, nu_b, nu_1):
	chi_AGN = 0
	model = (S_nu*f*(((frequency_obs/restfreq)**(-a_SA))/(1+(frequency_obs/(nu_b))**(0.5))) + (1-f)*S_nu*(frequency_obs/restfreq)**(-5/2)*(1-np.exp(-(frequency_obs/(nu_1))**(-a_SSA-5/2))))#*u.uJy
	for i in range(len(frequency_obs)):
		chi_AGN += ((flux[i]-model[i])/E_flux[i])**2
	red_chi_AGN = chi_AGN/(len(frequency_obs)-4)
	return red_chi_AGN
...
chi_SA_FF = red_chi_SA_FF(median_S_nu_SA_FF, median_f_th_SA_FF, median_a_SA_FF, median_nu_b_SA_FF)
chi_SA_FF_CO = red_chi_SA_FF_CO(median_S_nu_SA_FF_CO, median_f_th_SA_FF_CO, median_a_SA_FF_CO, median_nu_b_SA_FF_CO)
chi_AGN = red_chi_AGN(median_S_AGN, median_f_AGN, median_a_SA_AGN, a_SSA_AGN_guess, median_nu_b_AGN, nu_1_AGN_guess)
...
plt.plot(frequency, (median_f_th_SA_FF*median_S_nu_SA_FF*(frequency/restfreq)**(-0.1)+(1-median_f_th_SA_FF)*median_S_nu_SA_FF*((frequency/restfreq)**(-median_a_SA_FF))/(1+(frequency/(median_nu_b_SA_FF*u.GHz))**0.5))*u.uJy, '-',color=palette(6), label= r'SA + FF: $\chi^2_{red}$ = %.2f' % chi_SA_FF)
...
plt.plot(frequency, (1-median_f_th_SA_FF)*median_S_nu_SA_FF*((frequency/restfreq)**(-median_a_SA_FF))/(1+(frequency/(median_nu_b_SA_FF*u.GHz))**0.5)*u.uJy, '--',color=palette(6));

#Synchrotron aging +free free with exponential cutoff
plt.plot(frequency, (median_f_th_SA_FF_CO*median_S_nu_SA_FF_CO*(frequency/restfreq)**(-0.1)+(1-median_f_th_SA_FF_CO)*median_S_nu_SA_FF_CO*(frequency/restfreq)**(-median_a_SA_FF_CO)*np.exp(-frequency/(median_nu_b_SA_FF_CO*u.GHz)))*u.uJy, '-',color=palette(8), label= r'SA + FF,  exp: $\chi^2_{red}$ = %.2f' % chi_SA_FF_CO)
...
plt.plot(frequency, ((1-median_f_th_SA_FF_CO)*median_S_nu_SA_FF_CO*(frequency/restfreq)**(-median_a_SA_FF_CO)*np.exp(-frequency/(median_nu_b_SA_FF_CO*u.GHz)))*u.uJy, '--',color = palette(8));

#AGN Model
plt.plot(frequency, (median_S_AGN*median_f_AGN*(((frequency/restfreq)**(-median_a_SA_AGN))/(1+(frequency/(median_nu_b_AGN*u.GHz))**(0.5))) + (1-median_f_AGN)*median_S_AGN*(frequency/restfreq)**(-5/2)*(1-np.exp(-(frequency/(nu_1_AGN_guess*u.GHz))**(-a_SSA_AGN_guess-5/2))))*u.uJy, '-',color=palette(18), label= r'AGN model: $\chi^2_{red}$ = %.2f' % chi_AGN);
plt.fill_between(frequency, AGN_16, AGN_84, facecolor= palette(19), alpha=0.3, edgecolor = palette(19), interpolate = True);
...
plt.title('CXS-3 (z=0.688)', fontsize=22);
...
plt.savefig('CXS-3_Flux_Density.pdf',  dpi=500, clobber=True);
#plt.show();
...
axis1=[0.325*(1+z), 0.61*(1+z), 1.4*(1+z),3*(1+z),10*(1+z),22*(1+z)]
...
#Synchrotron aging + free-free
plt.plot(frequency*(1+z), Lumos((median_f_th_SA_FF*median_S_nu_SA_FF*(frequency/restfreq)**(-0.1)+(1-median_f_th_SA_FF)*median_S_nu_SA_FF*((frequency/restfreq)**(-median_a_SA_FF))/(1+(frequency/(median_nu_b_SA_FF*u.GHz))**0.5))*u.uJy), '-', color = palette(6),label= r'SA + FF: $\chi^2_{red}$ = %.2f' % chi_SA_FF)
...
plt.plot(frequency*(1+z), Lumos((1-median_f_th_SA_FF)*median_S_nu_SA_FF*((frequency/restfreq)**(-median_a_SA_FF))/(1+(frequency/(median_nu_b_SA_FF*u.GHz))**0.5)*u.uJy), '--',color = palette(6));

#Synchrotron aging +free free with exponential cutoff
plt.plot(frequency*(1+z), Lumos((median_f_th_SA_FF_CO*median_S_nu_SA_FF_CO*(frequency/restfreq)**(-0.1)+(1-median_f_th_SA_FF_CO)*median_S_nu_SA_FF_CO*(frequency/restfreq)**(-median_a_SA_FF_CO)*np.exp(-frequency/(median_nu_b_SA_FF_CO*u.GHz)))*u.uJy), '-',color=palette(8), label= r'SA + FF,  exp: $\chi^2_{red}$ = %.2f' % chi_SA_FF_CO)
...
plt.plot(frequency*(1+z), Lumos(((1-median_f_th_SA_FF_CO)*median_S_nu_SA_FF_CO*(frequency/restfreq)**(-median_a_SA_FF_CO)*np.exp(-frequency/(median_nu_b_SA_FF_CO*u.GHz)))*u.uJy), '--',color=palette(8));

#AGN Model
plt.plot(frequency*(1+z), Lumos((median_S_AGN*median_f_AGN*(((frequency/restfreq)**(-median_a_SA_AGN))/(1+(frequency/(median_nu_b_AGN*u.GHz))**(0.5))) + (1-median_f_AGN)*median_S_AGN*(frequency/restfreq)**(-5/2)*(1-np.exp(-(frequency/(nu_1_AGN_guess*u.GHz))**(-a_SSA_AGN_guess-5/2))))*u.uJy), '-',color = palette(18), label= r'AGN model: $\chi^2_{red}$ = %.2f' % chi_AGN);
plt.fill_between(frequency*(1+z), Lumos(AGN_16), Lumos(AGN_84), facecolor= palette(19), alpha=0.3, edgecolor = palette(19), interpolate = True);
...
plt.savefig('CXS-3_Luminosity.pdf',  dpi=500, clobber=True);
#plt.show();
...
#Some logistics of the figure
#plt.title('CXS-3 (z=1.44)', fontsize=15);
plt.xlabel(r'$\nu_{rest}$ (GHz)', fontsize = 22);
plt.ylabel(r'Luminosity (W Hz$^{-1}$)', fontsize=22);
plt.xscale('log');
plt.yscale('log');
...
plt.yticks(fontsize=20)
plt.xticks(axis1,axis1,fontsize=20)
plt.savefig('CXS-3_pl.pdf',  dpi=500, clobber=True);
#plt.show();
...
plt.yticks(fontsize=20)
plt.xticks(axis1,axis1,fontsize=20)
plt.savefig('CXS-3_FF+S.pdf',  dpi=500, clobber=True);
#plt.show();
...
plt.yticks(fontsize=20)
plt.xticks(axis1,axis1,fontsize=20)
plt.savefig('CXS-3_SA.pdf',  dpi=500, clobber=True);
#plt.show();
...
plt.yticks(fontsize=20)
plt.xticks(axis1,axis1,fontsize=20)
plt.savefig('CXS-3_SA+FF.pdf',  dpi=500, clobber=True);
#plt.show();
...
plt.yticks(fontsize=20)
plt.xticks(axis1,axis1,fontsize=20)
plt.savefig('CXS-3_SA+FF+exp.pdf',  dpi=500, clobber=True);
#plt.show();
...
#Some logistics of the figure
#plt.title('CXS-3 (z=0.688)', fontsize=15);
plt.xlabel(r'$\nu_{rest}$ (GHz)', fontsize = 22);
plt.ylabel(r'Luminosity (W Hz$^{-1}$)', fontsize=22);
plt.xscale('log');
plt.yscale('log');
...
plt.yticks(fontsize=20)
plt.xticks(axis1,axis1,fontsize=20)
plt.savefig('CXS-3_AGN.pdf',  dpi=500, clobber=True);
plt.show();
